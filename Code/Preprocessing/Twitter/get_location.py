...
def write_dict_to_csv(name,dict,*columns):
    ...
    for key, val in dict.items():
        ...
        if(type(val)==list):
            ...
            path = "C:\\Users\\Administrator\\Desktop\\diplwmatikh giwrgos fragkozidhs\\train\\Counties\\" + county + "\\" + str(year) + "\\" + str(week) + "\\" + "output.csv"
            print(path)
            ...
            myfile = open("C:\\Users\\Administrator\\Desktop\\diplwmatikh giwrgos fragkozidhs\\train\\Counties\\output_info.csv", "a+",encoding='utf8')  
            w = csv.writer(myfile)
            if(columns):
                w.writerow( *columns)
            keys[key]=True
